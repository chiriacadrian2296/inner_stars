...
import '../utils/physical_art_tint.dart';
import 'constellation_field.dart';

/// An early look at using [LifeArea.physical]'s own hero art (see
/// assets/images) as part of the sky itself, not just its detail page — a
/// huge, faint wash of the image sitting behind that area's own supernova.
...
///
/// Only [LifeArea.physical] has art of its own so far, so this paints
/// nothing for any other area.
...
class _SkyAreaBackdropState extends State<SkyAreaBackdrop> {
  static const _asset = 'assets/images/1. Physical - CENTER CROP.jpg';
...
/// reason: without it, this backdrop would sit screen-aligned no matter how
/// the camera moves, rather than reading as pinned to the sky at
/// [LifeArea.physical]'s own spot the way a real patch of nebula would be.
...
/// What this backdrop's own `rotation` (see [_SkyAreaBackdropPainter.paint])
/// would read as under [camera] — reusing [_rotationAngleFor] itself rather
/// than a separately hand-derived formula, at an arbitrary unit probe size
/// (the angle it returns doesn't depend on zoom/screen size, only on
/// direction). `sky_screen.dart`'s own hold-to-straighten flight (see
/// `_holdArea`) calls this — once with the camera as it will be *before* any
/// roll correction, once as the target 0 — to work out how much roll would
/// land the image reading straight, the same way it already straightens a
/// constellation's own shape via `cameraRollAngle`
/// (constellation_field.dart) targeting a different canonical reading.
/// Meaningless for any camera not already looking dead-center at
/// [LifeArea.physical]'s own supernova — every caller only ever uses this
/// after a pan-only [SkyCamera.rotatedToAlign] onto that exact direction.
double physicalBackdropRollReading(SkyCamera camera) {
  final direction = supernovaDirection(
    LifeArea.physical.index,
...
class _SkyAreaBackdropPainter extends CustomPainter {
  ...
  static const _worldRadius = 0.3;

  // Additive blend already turns the image's black background invisible;
  ...
  // Where the circular vignette (see [paint]) starts fading from fully
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // what makes it read as pinned to the sky at Physical's own spot
    // instead of a flat billboard that stays upright no matter where the
    // camera looks.
    final axisA = _axisAFor(direction);
    final rotation = _rotationAngleFor(direction, axisA, center, camera, zoom, size);
    ...
    canvas.drawImageRect(
      ...
      Paint()
        ...
        ..colorFilter = kPhysicalHeroArtTint,
    );
